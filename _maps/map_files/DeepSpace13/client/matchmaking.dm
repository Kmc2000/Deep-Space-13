//Ported from BayStation12, allows for inter-character relationships as an opt-in feature
...
/datum/relation/New()
	...
	SSjob.matchmaker.relations += src

/datum/relation/proc/get_candidates()
	.= list()
	for(var/datum/relation/R in SSjob.matchmaker.relations)
...
/datum/relation/proc/valid_candidate(datum/mind/M)
	...
	if(!isnull(M.antag_datums))
		return FALSE

	return TRUE

/datum/relation/proc/can_connect(var/datum/relation/R)
	for(var/datum/relation/D in SSjob.matchmaker.relations) //have to check all connections between us and them
...
/datum/relation/proc/sever()
	...
	other.other = null
	SSjob.matchmaker.relations -= other
	SSjob.matchmaker.relations -= src
	qdel(other)
	other = null
	qdel(src)
...
/mob/living/verb/see_relationship_info()
	...
	var/list/relations = SSjob.matchmaker.get_relationships(mind)
	var/list/dat = list()
	var/editable = 0
	if(mind.gen_relations_info)
		...
		dat += "<br>"
	for(var/datum/relation/R in relations)
		dat += "<b>[R.other.finalized ? "\[F\] " : ""][R.other.holder]</b>, [R.other.holder.assigned_role]."
		if (!R.finalized)
...
/mob/living/proc/see_relationship_info_with(var/mob/living/other)
	if(!other.mind)
		return
	var/list/relations = SSjob.matchmaker.get_relationships(mind,other.mind,TRUE)
